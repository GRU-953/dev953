...
# Clear any LEFTOVER lock from a crashed prior run (no-op if none), then acquire.
# `release` is no-op-safe; `acquire` creates .dev953/ and the lock atomically and
# REFUSES (nonzero) only if a genuinely live lock still remains.
node "$PLUGIN/skills/memory/lease.mjs" run-lock release
