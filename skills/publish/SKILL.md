...
2. **Scan.** INVOKE the secret scan — do not reimplement it. Run a push-capable
   command shape inside the temp clone so the `scan.mjs` PreToolUse hook
   (`hooks/scan.mjs`, matcher Bash) fires and scans the would-ship set (committed +
...
9. **Push, then STOP.** TRIGGER the confirm-push card. On the user's yes,
   `git push -u origin main` to the fresh remote. The `scan.mjs` PreToolUse hook
...
10. **Explicit public gate (separate, opt-in, NEVER automatic).** Only on an
    ...
    re-INVOKE `scan.mjs`, re-verify identity, re-confirm the specific repo name,
